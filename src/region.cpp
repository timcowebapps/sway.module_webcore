...
void Region::registerEmscriptenClass(lpcstr_t classname) {
	emscripten::class_<Region>(classname)
		.constructor<core::containers::Hierarchy *, core::containers::HierarchyNodeIdx, RegionCreateInfo>()
...
Region::Region(core::containers::Hierarchy * tree,
	const core::containers::HierarchyNodeIdx & parentNodeIndex, const RegionCreateInfo & createInfo)
	: _tree(tree)
...
void Region::attachView(TreeNodeElement * node) {
	_attachedNodeId = node->getNodeUid();
	_attached = _tree->findNode<webcore::TreeNodeElement>(_parentNodeIndex)->addChild(node).isValid();
}

void Region::detachView(TreeNodeElement * node) {
	if (_attached) {
		_tree->findNode<webcore::TreeNodeElement>(_parentNodeIndex)->removeChild(node);
		_attached = false;
	}
}
